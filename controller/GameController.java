...
public class GameController {
    ...
    private static boolean move(TerrainTile start, TerrainTile end) {
        if (!(end.isEmpty() && GridFX.adjacent(end, start)
            ...
            if (!(start.getOccupant().getOwner() instanceof Bandit)) {
                Alert attackAlert = new Alert(Alert.AlertType.WARNING);
                attackAlert.setTitle("Warning");
                attackAlert.setHeaderText("Unit Move Failure");
                attackAlert.setContentText("Your Unit cannot be moved there.");
    ...
    private static void attackTile(TerrainTile attacker, TerrainTile enemy) {
        if (enemy.isEmpty()
            ...
            if (!(attacker.getOccupant().getOwner() instanceof Bandit)) {
                Alert attackAlert = new Alert(Alert.AlertType.WARNING);
                attackAlert.setTitle("Warning");
                attackAlert.setHeaderText("Attack Alert");
                attackAlert.setContentText("Your attack cannot be completed!");
                attackAlert.showAndWait();
            }
            return;
        }
