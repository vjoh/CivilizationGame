...
public class TerrainTileFX extends StackPane {
     ...
     * It should check if the TerrainTile is empty.
     *
     * If it is empty,
     * set the overlay to be transparent.
     *
     * If it is not empty, fill
     * the overlay with the color of the occupant on the terrain tile
     *
     * If the TerrainTileFX contains an icon, remove it.
     *
     * If the tile is not empty, get the image of the occupant of the tile and
     * add the image of the occupant to the tile.
     */
    public void updateTileView() {
        ...
        if (GameController.getLastClicked() == this) {
            overlay.setFill(Color.web("#fceb05", 0.65));
        }
